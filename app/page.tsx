  ...
  return (
    <main onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); importFile(event.dataTransfer.files[0]); }}>
      ...
      <section className="hero" id="top">
        ...
        <p className="lede">Turn raw agent JSONL logs into a navigable workflow of phases, decisions, tools, files, recovery chains, token efficiency, regressions, and quality gates—then bookmark or restart from any exact checkpoint.</p>
      ...
      <section className="how" id="why">
        ...
        <div className="steps"><article><b>01</b><h3>Normalize</h3><p>Loose JSON and JSONL shapes become one event model without vendor lock-in.</p></article><article><b>02</b><h3>Reconstruct</h3><p>See the run move through inspection, implementation, verification, and publishing.</p></article><article><b>03</b><h3>Diagnose</h3><p>Link failures to successful retries and expose unresolved operations, loops, and stalls.</p></article><article><b>04</b><h3>Compare</h3><p>Measure a new run against a baseline with normalized regression and efficiency evidence.</p></article><article><b>05</b><h3>Gate</h3><p>Make CI fail on explicit quality, recovery, or token limits, missing proof, or a regression.</p></article><article><b>06</b><h3>Resume</h3><p>Bookmark exact checkpoints, share deep links, or export context to continue safely.</p></article></div>
