  ...
  return (
    <main onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); importFile(event.dataTransfer.files[0]); }}>
      ...
      <section className="how" id="why">
        ...
        <div className="terminal"><div><span /><span /><span /></div><code><i>$</i> backtrace-agent current.jsonl --audit-ingestion<br /><em>Semantic coverage: 100.0% · unsupported types: none</em><br /><i>$</i> backtrace-agent current.jsonl <br />&nbsp;&nbsp;--find deploy_private_site_version --find-limit 5<br /><em>Matches: 5 of 28 · stable event IDs returned</em><br /><i>$</i> backtrace-agent current.jsonl --incident failed-42<br /><em>Incident: test · recovered · 2 failed attempts</em></code></div>
      </section>
