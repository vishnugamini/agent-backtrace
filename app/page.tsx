  ...
  return (
    <main onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); importFile(event.dataTransfer.files[0]); }}>
      ...
      <section className="hero" id="top">
        ...
        <p className="lede">Search any action, catch parser drift, recover complete retry chains, then export focused evidence cryptographically tied to its exact source.</p>
      ...
      <section className="how" id="why">
        ...
        <div className="steps"><article><b>01</b><h3>Normalize</h3><p>Loose JSON and JSONL shapes become one event model without vendor lock-in.</p></article><article><b>02</b><h3>Audit</h3><p>Separate bookkeeping from meaningful records and expose unsupported provider types before they vanish silently.</p></article><article><b>03</b><h3>Search</h3><p>Find any command, file, deployment, message, or checkpoint without generating a report.</p></article><article><b>04</b><h3>Focus</h3><p>Point at one failure and automatically capture every retry through recovery, plus context.</p></article><article><b>05</b><h3>Gate</h3><p>Fail CI on regressions, missing proof, risky actions, or parser drift.</p></article><article><b>06</b><h3>Prove</h3><p>Bind sanitized evidence to the exact source trace without placing that private trace in the bundle.</p></article></div>
        <div className="terminal"><div><span /><span /><span /></div><code><i>$</i> backtrace-agent current.jsonl --audit-ingestion<br /><em>Adapter coverage: 100.0% · unsupported types: none</em><br /><em>Reasoning: supported · empty content tracked separately</em><br /><i>$</i> backtrace-agent current.jsonl <br />&nbsp;&nbsp;--find deploy_private_site_version --find-limit 5<br /><em>Matches: 5 of 28 · stable event IDs returned</em><br /><i>$</i> backtrace-agent current.jsonl --incident failed-42<br /><em>Incident: test · recovered · 2 failed attempts</em></code></div>
      </section>
