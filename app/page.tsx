  ...
  return (
    <main onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); importFile(event.dataTransfer.files[0]); }}>
      ...
      <section className="how" id="why">
        ...
        <div className="steps"><article><b>01</b><h3>Normalize</h3><p>Loose JSON and JSONL shapes become one event model without vendor lock-in.</p></article><article><b>02</b><h3>Diagnose</h3><p>An attention queue surfaces failures, repetition, stalls, slow actions, and missing proof.</p></article><article><b>03</b><h3>Compare</h3><p>Measure a new run against a baseline with normalized regression and improvement evidence.</p></article><article><b>04</b><h3>Resume</h3><p>Pick a checkpoint and export the context an agent needs to continue safely.</p></article></div>
        <div className="terminal"><div><span /><span /><span /></div><code><i>$</i> pip install git+https://github.com/vishnugamini/agent-backtrace.git<br /><i>$</i> backtrace-agent current.jsonl --compare baseline.jsonl -o report.html<br /><em>Comparison: improved · 1 regression · 4 improvements<br />Report: report.html</em></code></div>
      </section>
