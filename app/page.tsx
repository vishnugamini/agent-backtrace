  ...
  return (
    <main onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); importFile(event.dataTransfer.files[0]); }}>
      ...
      <section className="hero" id="top">
        ...
        <p className="lede">Watch a live agent trace, isolate one failure or agent, inspect recovery and side effects, then export focused evidence cryptographically tied to its exact source.</p>
      ...
      <section className="how" id="why">
        ...
        <div className="steps"><article><b>01</b><h3>Normalize</h3><p>Loose JSON and JSONL shapes become one event model without vendor lock-in.</p></article><article><b>02</b><h3>Map</h3><p>Plot every agent, event, handoff, file change, and failure on one shared clock.</p></article><article><b>03</b><h3>Focus</h3><p>Extract an exact event window or one agent without mislabeling session-wide metrics.</p></article><article><b>04</b><h3>Diagnose</h3><p>Link failures to retries and audit pushes, deployments, installs, and destructive actions.</p></article><article><b>05</b><h3>Gate</h3><p>Apply a checked-in policy and publish each result to JUnit-compatible CI reports.</p></article><article><b>06</b><h3>Prove</h3><p>Bind sanitized evidence to the exact source trace without placing that private trace in the bundle.</p></article></div>
        <div className="terminal"><div><span /><span /><span /></div><code><i>$</i> backtrace-agent current.jsonl <br />&nbsp;&nbsp;--from-event failed-42 --to-event recovered-57<br />&nbsp;&nbsp;--agent codex --bundle focused.zip<br /><em>Focus: 5/678 events · agents: codex</em><br /><i>$</i> backtrace-agent --verify-bundle focused.zip <br />&nbsp;&nbsp;--verify-source current.jsonl<br /><em>Bundle verification: PASS · source trace matched</em></code></div>
      </section>
