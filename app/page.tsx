  ...
  return (
    <main onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); importFile(event.dataTransfer.files[0]); }}>
      ...
      <section className="hero" id="top">
        ...
        <p className="lede">Catalog every failure, point at one to recover its complete retry chain, then export focused evidence cryptographically tied to its exact source.</p>
      ...
      <section className="how" id="why">
        ...
        <div className="terminal"><div><span /><span /><span /></div><code><i>$</i> backtrace-agent current.jsonl --list-incidents<br /><em>11 incidents · 1 unresolved · 10 recovered</em><br /><i>$</i> backtrace-agent current.jsonl <br />&nbsp;&nbsp;--incident failed-42 --context-events 2<br />&nbsp;&nbsp;--bundle incident.zip<br /><em>Incident: test · recovered · 2 failed attempts</em></code></div>
      </section>
