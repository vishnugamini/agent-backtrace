  ...
  return (
    <main onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); importFile(event.dataTransfer.files[0]); }}>
      ...
      <section className="hero" id="top">
        ...
        <p className="lede">Watch a live agent trace, inspect recovery, side effects, efficiency, and regressions, enforce a checked-in policy, then export evidence cryptographically tied to its exact source.</p>
