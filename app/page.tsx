  ...
  return (
    <main onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); importFile(event.dataTransfer.files[0]); }}>
      ...
      <section className="hero" id="top">
        ...
        <p className="lede">Prove the source is healthy, catch parser drift, recover complete retry chains, then export focused evidence tied to the exact trace.</p>
      ...
      <section className="how" id="why">
        ...
        <div className="terminal"><div><span /><span /><span /></div><code><i>$</i> backtrace-agent current.jsonl --doctor<br /><em>Trace doctor: HEALTHY · jsonl</em><br /><em>Integrity issues: 0 · concurrent ordering notes reported separately</em><br /><i>$</i> backtrace-agent current.jsonl --audit-ingestion<br /><em>Adapter coverage: 100.0% · unsupported types: none</em><br /><i>$</i> backtrace-agent current.jsonl --incident failed-42<br /><em>Incident: test · recovered · 2 failed attempts</em></code></div>
      </section>
